...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        adminAuth.checkAdmin().then(setIsAdmin);
        const { data } = adminAuth.onAuthStateChange(setIsAdmin);
    ...
    const handleAdminLogin = async (e: React.FormEvent) => {
        ...
        try {
            const success = await adminAuth.login(adminPassword);
            if (success) {
                setIsAdmin(true);
                ...
                setAdminError('Invalid admin password or admin account is not allowed.');
            }
        } catch (error: any) {
