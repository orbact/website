...
const SolutionDetail: React.FC = () => {
   ...
   const addToRefs = useScrollReveal([id, service]);

   useEffect(() => {
      async function loadService() {
         if (!id) return;
         setLoading(true);
         const cmsService = await serviceService.getServiceBySlug(id);
         const fallback = SERVICES.find(s => s.id === id || s.slug === id) || null;
         const activeService = cmsService || fallback;
         setService(activeService);

         if (activeService) {
            const projects = await projectService.getAllProjects();
            const keyword = activeService.title.split(' ')[0].toLowerCase();
            setRelatedCases(projects.filter(project => project.category.toLowerCase().includes(keyword)).slice(0, 2));
         }
         setLoading(false);
      }
   ...
   if (loading) {
      return (
         <div className="container mx-auto px-6 py-32 text-center text-muted">
            Loading service...
         </div>
      );
   }
